...
int main()
{
    ...
    *****************************************************************************************/
	...
	cout<<"Initialize lattice as empty"<<endl;
    ...
    /**************************************************************************************
                                        Read initial material file
    *****************************************************************************************/
	// Initialization for all iterations of properties
    cout<<"Read initial material properties"<<endl;
    Read_material_file(Young_modulus, Poisson_ratio); 
    ...
    *****************************************************************************************/
    ...
    Initialize_lattice(lattice, age_cells, lattice_point_element, Young_modulus);                    
    
    /**************************************************************************************
                                     Write_lattice_file_for_gnuplot
    *****************************************************************************************/
    cout<<"Write results file"<<endl;
    iteration=-1; // Use -1 for initial setup to avoid accessing invalid array indices
    ...
    // Write_cell_count_results(lattice, iteration); // cell count results
    Write_cell_count_summary(lattice, iteration); // cell count summary
    ...
    for (iteration=0;iteration<NUMBER_ITERATIONS;iteration++)
    {
        ...
        *****************************************************************************************/
        ...
        /**************************************************************************************
                                              Update model
        *****************************************************************************************/  
        if (iteration<NUMBER_ITERATIONS && iteration%3==0) // no need to update model in the last iteration; ; run every 3 iterations
        {
            cout<<"Update model"<<endl;
            Update_model(lattice, Young_modulus, Poisson_ratio, iteration, element_local_min, element_local_max, lattice_point_element);
            Write_youngs_modulus_histogram(Young_modulus, iteration, 100.0f); // Young's modulus histogram with xxx MPa bins
        }

        /**************************************************************************************
                                           Increase cell age
        *****************************************************************************************/               
		if (iteration<NUMBER_ITERATIONS-1) // no need to increase cell age in the last iteration
		{
        ...
        /**************************************************************************************
                                              Run abaqus
        *****************************************************************************************/  
        if (iteration<NUMBER_ITERATIONS-1 && iteration%3==0) // no need to run ABAQUS model in the last iteration; run every 3 iterations
        {
            cout<<"run abaqus"<<endl;
            system("run.bat"); // ./run.bat in LINUX; run.bat in WINDOWS
        }
