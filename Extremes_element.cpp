...
//        cout<<"zmax: "<<zmax<<endl;   
        ...
        // Clamp bounds to valid array indices to prevent out-of-bounds access
        if (extreme_min[elem][0] < 0) extreme_min[elem][0] = 0;
        if (extreme_min[elem][1] < 0) extreme_min[elem][1] = 0;
        if (extreme_min[elem][2] < 0) extreme_min[elem][2] = 0;
        if (extreme_max[elem][0] >= LATTICE_X) extreme_max[elem][0] = LATTICE_X - 1;
        if (extreme_max[elem][1] >= LATTICE_Y) extreme_max[elem][1] = LATTICE_Y - 1;
        if (extreme_max[elem][2] >= LATTICE_Z) extreme_max[elem][2] = LATTICE_Z - 1;
