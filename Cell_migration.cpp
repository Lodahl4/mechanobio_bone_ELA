/************************************************************
    Algorithm to migrate cells
        7 jumps each iteration (for lattice point size 100�m)
        (30 micrometers/hour)
	...
	int lattice_points_element[LATTICE_X][LATTICE_Y][LATTICE_Z], int iter)
{
    ...
    // int a,b,h;
	// h = int(4/CELL_DIAMETER)+1;		// bone extremity is 4 mm high
    // a = int(2/CELL_DIAMETER)+1; // long axis bone marrow
	// b = int(2/CELL_DIAMETER)+1; // small axis bone marrow
    for (i=0;i<LATTICE_X;i++) {
        for (j=0;j<LATTICE_Y;j++) {
            for (k=0;k<LATTICE_Z;k++) {
                if (cells_migration[i][j][k]==1) { // it is an MSC
					if (lattice_points_element[i][j][k]!=0) { // || ((k<h || k>=LATTICE_Z-h) && (pow(i-(LATTICE_X-1)/2,2)/pow(a,2)+pow(j-(LATTICE_Y-1)/2,2)/pow(b,2)<=1))) { 	// it is in an element or in the marrow cavity
						MSCs += 1;
						Point temp = {i,j,k};
...
//    cout << "    Number of MSCs: " << MSCs << endl;
	...
	std::shuffle(indices.begin(), indices.end(), rng); // shuffle the vector of indices
	...
	for(Point& p:indices) {
	    ...
	    if (cells_migration[i][j][k]==1) {    
		    if (lattice_points_element[i][j][k]!=0) { // || ((k<h && h-k<=h/3 || k>=LATTICE_Z-h && k-LATTICE_Z+h<h/3) && (pow(i-(LATTICE_X-1)/2,2)/pow(a,2)+pow(j-(LATTICE_Y-1)/2,2)/pow(b,2)<=1))) {  
    ...
    ofstream myfile;
	myfile.open("output\\migration_distribution.txt", ios::app);
	for (i=0; i<max_jumps; i++) {
